...
class TriesNode {
  ...
  class Tries {
    ...
    getWords(prefix: string): string[] {
      ...
      this._getWords(lastNode, prefix, words);
      return words;
    }
  
    private _getWords(node: TriesNode, prefix: string, words: string[]) {
      // Guard for no prefix match 
      if (!node) return;
      ...
      for (let child of node.getChildren()) {
        // appending char to prefix, ie "car" + "e" = "care"
        this._getWords(child, prefix + child.character, words);
      }
    }
  
    private findLastNodeOf(prefix: string): TriesNode | null {
